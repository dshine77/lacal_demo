...
/**
 *  自定义的堆结构，可以实现当堆结构已经建立之后， 修改对象的某一个属性值，可以重新调整堆
 *  java系统的堆无法实现这样的调整。***这样的调整只对非基本对象有用
 *
 *
 *  对象的比较规则通过定义比较器设定
 *
...
public class Code03_Heap02 {
	...
	public static class MyHeap<T> {
		...
		//使用一个map 保存每个对象的位置，以便于对对象做修改的时候可以通过对象的引用找到该对象的位置，然后重新调整堆
		//key:对象的引用（内存地址）  value: 在堆上的位置
